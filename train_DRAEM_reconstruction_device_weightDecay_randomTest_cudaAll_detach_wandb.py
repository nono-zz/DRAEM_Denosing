...
def train_on_device(args):
    ...
    optimizer_denoise = torch.optim.Adam(model_denoise.parameters(), lr = args.lr)
    scheduler_denoise = torch.optim.lr_scheduler.StepLR(optimizer=optimizer_denoise, step_size=100, gamma=0.1)
    
    optimizer_segment = torch.optim.Adam(model_segment.parameters(), lr = args.lr)
    scheduler_segment = torch.optim.lr_scheduler.StepLR(optimizer=optimizer_segment, step_size=100, gamma=0.1)
    ...
    for epoch in range(last_epoch, args.epochs):
        ...
        for img, aug, anomaly_mask in tqdm(train_dataloader):
            ...
            loss_rec_l2 = loss_l2(rec,img)
            # loss_rec_ssim = loss_ssim(rec, img)
            
            # loss_rec = loss_rec_l2 + loss_rec_ssim
            loss_rec = loss_rec_l2
            optimizer_denoise.zero_grad()
            loss_rec.backward()
            optimizer_denoise.step()
            
            # segmentation
            seg_input = rec.detach()
            joined_in = torch.cat((seg_input, aug), dim=1)
            
            out_mask = model_segment(joined_in)
            out_mask_sm = torch.softmax(out_mask, dim=1)
            
            loss_segment = loss_focal(out_mask_sm, anomaly_mask)
            ...
            loss = loss_rec + loss_segment
            loss_list.append(loss.item())
            loss_rec_list.append(loss_rec.item())
            loss_seg_list.append(loss_segment.item())
            

        scheduler_denoise.step()
        scheduler_segment.step()
        
        current_learning_rate = get_lr(optimizer_segment)
        print("current learning rate is:   ", current_learning_rate)
        
        print('epoch [{}/{}], loss:{:.6f}, loss_rec:{:.6f}, loss_seg:{:.6f} \n'.format(args.epochs, epoch, mean(loss_list), mean(loss_rec_list), mean(loss_seg_list)))
...
if __name__=="__main__":
    ...
    parser.add_argument('--epochs', default=450, action='store', type=int)
    ...
    parser.add_argument('--experiment_name', default='DRAEM_Denoising_reject_weightDecay_detach_wo_ssim', choices=['DRAEM_Denoising_reconstruction, liver, brain, head'], action='store')
